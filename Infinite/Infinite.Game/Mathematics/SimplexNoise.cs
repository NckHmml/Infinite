...
namespace Infinite.Mathematics
{
    ...
    public class SimplexNoise
    {
        ...
        readonly static double F2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
        readonly static double G2 = (3.0 - Math.Sqrt(3.0)) / 6.0;
        ...
        private static double grad(int hash, double x, double y)
        {
            int h = hash & 7;
            if (h < 4)
                return ((h & 1) != 0 ? -x : x) + ((h & 2) != 0 ? -2.0 * y : 2.0 * y);
            else
                return ((h & 1) != 0 ? -y : y) + ((h & 2) != 0 ? -2.0 * x : 2.0 * x);
        }
